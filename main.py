...
df_main = pd.read.csv("game-ratings-by-release-dates.csv")
...
plt.scatter(df_main["first_release_date"], df_main["user_rating_value"], color = "purple", label = "User Ratings")
...
df_follow = df_follow.rename(columns = { "follow_cout": "total_follows" })
...
plt.bar(df_hype.index + BAR_WIDTH / 2, df_hype["total_hypes"], color = "purple", label = "Number of Hypes", width = BAR_WIDTH)
